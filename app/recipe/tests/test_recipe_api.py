...
class PrivateRecipeApiTests(TestCase):
    ...
    def test_partial_update(self):
        ...
        org_link = 'https://example.com/recipe.pdf'
    ...
    def test_create_recipe_with_new_tags(self):
        ...
        payload = {
            'title': 'Malaysian Parota',
            'time_minutes': 40,
            'price': Decimal('90.0'),
            'tags': [{'name': 'Malaysian'}, {'name': 'Dinner'}],
        }
    ...
    def test_create_recipe_with_existing_tags(self):
        ...
        self.assertEqual(recipes.count(), 1)
        self.assertEqual(recipes[0].tags.count(), 2)
        self.assertIn(tag_italian, recipes[0].tags.all())
        for tag in payload['tags']:
            exists = recipes[0].tags.filter(
                name=tag['name'],
                user=self.user,
    ...
    def test_create_tag_on_recipe_update(self):
        """Test creating tag on updating recipe with new tag"""
        recipe = create_recipe(user=self.user)
    ...
    def test_update_tag_on_recipe_update(self):
        """Test assigning existing tag on updating recipe tag"""
    ...
    def test_create_recipe_with_new_ingredients(self):
        ...
        payload = {
            ...
            'time_minutes': 40,
            'price': Decimal('90.0'),
            'ingredients': [{'name': 'Garlic'}, {'name': 'Salt'}],
        }
        res = self.client.post(RECIPES_URL, payload, format='json')
    ...
    def test_create_recipe_with_existing_ingredients(self):
        ...
        ingredient_garlic = Ingredient.objects.create(
            user=self.user, name='Garlic')
        payload = {
            'title': 'Italian Pizza',
            'time_minutes': 40,
            'price': Decimal('90.0'),
            'ingredients': [{'name': 'Garlic'}, {'name': 'Tomato'}],
        }
        res = self.client.post(RECIPES_URL, payload, format='json')
        ...
        self.assertEqual(recipes.count(), 1)
        self.assertEqual(recipes[0].ingredients.count(), 2)
        self.assertIn(ingredient_garlic, recipes[0].ingredients.all())
        for ingredient in payload['ingredients']:
            exists = recipes[0].ingredients.filter(
                name=ingredient['name'],
                user=self.user,
    ...
    def test_create_ingredient_on_recipe_update(self):
        """Test creating ingredient on updating recipe with new ingredient"""
        recipe = create_recipe(user=self.user)
        ...
        new_ingredient = Ingredient.objects.get(
            user=self.user, name='Spinach')
        self.assertIn(new_ingredient, recipe.ingredients.all())

    def test_update_ingredient_on_recipe_update(self):
        """Test assigning existing ingredient on updating recipe ingredient"""
    ...
    def test_filter_by_tags(self):
        ...
        r1 = create_recipe(user=self.user, title='Mysore Pak')
        r2 = create_recipe(user=self.user, title='Palak Paneer')
        tag1 = Tag.objects.create(user=self.user, name='Sweets')
        tag2 = Tag.objects.create(user=self.user, name='Paneer')
        r1.tags.add(tag1)
        r2.tags.add(tag2)
        r3 = create_recipe(user=self.user, title='Butter Roti')

        params = {'tags': f'{tag1.id},{tag2.id}'}
        res = self.client.get(RECIPES_URL, params)

        s_r1 = RecipeSerializer(r1)
        s_r2 = RecipeSerializer(r2)
        s_r3 = RecipeSerializer(r3)
        self.assertIn(s_r1.data, res.data)
        self.assertIn(s_r2.data, res.data)
        self.assertNotIn(s_r3.data, res.data)
